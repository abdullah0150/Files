...
EXCEPT
SELECT CustomerID, Gender, SeniorCitizen, Partner, Dependents
FROM TargetDatabase.dbo.DimCustomer;
...
SELECT TOP 5 * FROM FactCustomerInteraction;
...
CREATE VIEW vw_Avg_Monthly_Charges AS
SELECT 
...
CREATE VIEW vw_Avg_Customer_Lifetime_Duration AS
SELECT 
...
CREATE VIEW vw_Avg_Monthly_Charges_By_Contract AS
SELECT 
    dc.Contract,
...
CREATE VIEW vw_Avg_Tenure_By_Payment_Method AS
SELECT 
    dp.PaymentMethod,
...
CREATE VIEW vw_CLV_By_Payment_Method AS
SELECT 
    dp.PaymentMethod,
...
CREATE VIEW vw_CLV_By_Tenure AS
SELECT 
    CASE
...
CREATE VIEW vw_Contract_Type_Distribution AS
SELECT 
    c.Contract,
    COUNT(*) AS Total_Customers,
    (COUNT(*) * 1.0 / (SELECT COUNT(*) FROM FactCustomerInteraction)) * 100 AS Percentage
...
CREATE VIEW vw_Internet_Service_Distribution AS
SELECT 
    is_.InternetService,
    COUNT(*) AS Total_Customers,
    (COUNT(*) * 1.0 / (SELECT COUNT(*) FROM FactCustomerInteraction)) * 100 AS Percentage
...
CREATE VIEW vw_Churn_Status_Distribution AS
SELECT 
    fci.Churn,
    COUNT(*) AS Total_Customers,
    (COUNT(*) * 1.0 / (SELECT COUNT(*) FROM FactCustomerInteraction)) * 100 AS Percentage
